'STATS GATHERING----------------------------------------------------------------------------------------------------
name_of_script = "ACTIONS - MFIP SUPPLEMENT.vbs"
start_time = timer
...
Call navigate_to_MAXIS_screen("ELIG", "MFB2")
EMReadScreen supplement_due, 8, 9, 74
supplement_due = trim(supplement_due)
If supplement_due = "0.00" then script_end_procedure("A supplement is not needed for this case. Please review the footer month/year selected and the case.")
'transmiting into the 'supplement screen'
EMWriteScreen "x", 9, 44
transmit
EMReadScreen food_supplement, 8, 11, 33	'reading supplement amounts and cleaning up variables
food_supplement = trim(food_supplement)
EMReadScreen cash_supplement, 8, 11, 70
cash_supplement = trim(cash_supplement)
EMReadScreen state_food_supplement, 8, 16, 33
state_food_supplement = trim(state_food_supplement)
EMReadScreen housing_grant_supplement, 8, 16, 70
housing_grant_supplement = trim(housing_grant_supplement)
...
'establishing value for varialbes to search for in INQX
If food_supplement <> "0.00" then program_to_seach = "FS"
IF cash_supplement <> "0.00" then program_to_seach = "MF"
If state_food_supplement <> "0.00" then program_to_seach = "FS"
If housing_grant_supplement <> "0.00" then program_to_seach = "HG"

'checking to see if HG has been issued for the month selected	
DO
	row = 6				'establishing the row to start searching for issuance'
	DO
		EMReadScreen program_to_seach, 2, row, 19		'searching for housing grant issuance
		If program_to_seach = "  " then exit do
		IF program_to_seach = "HG" then
